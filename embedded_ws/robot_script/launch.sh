MAIN_BRANCH="develop"
export MISSION_STATUS="EN ATTENTE"
...
if [ -d ~/geppetto/ ]; then
    ...
    cd ~; git clone https://github.com/thomas-rou/Geppetto.git
    cd ~/geppetto/; git switch $MAIN_BRANCH; git pull
fi
cd ~/limo_ws; colcon build; source install/setup.bash
cd ~/limo_ws/src/limo_ros2; ros2 launch limo_bringup limo_start.launch.py &
cd ~/geppetto/embedded_ws/; colcon build;  source install/setup.bash;  ros2 run com_bridge com_serv & ros2 run mission_status_manager mission_manager
